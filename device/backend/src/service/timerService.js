...
class TimerService {
  ...
  async init(){
    ...
    rows.forEach((item)=>{
      const component = new Timer(item.timer_key, item.group_key, item.init_time, item.max_iter);
      this.array.set(item.timer_key, component);
    })
  }
  async createTimer(initTimes, maxIter=1, group_id=0) {
    const id = await this.timerRepository.insert(group_id, initTimes, maxIter);
