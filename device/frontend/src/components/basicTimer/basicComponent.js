...
export default function BasicTimerComponent({ timer, idx, type, removeTimer, WatchId  }) {
  ...
  return (
    <Box container className={type=="timer"?"watch timer":"watch stopWatch"}>
      <Grid container xs={100} justifyContent={"center"} alignContent={"center"}>
        <Grid xs={1}><h2>{idx+1}</h2></Grid>
        ...
        <Grid>
          <IconButton  ton aria-label="delete" variant="text" color="error" size="large" onClick={remove}>
            <CloseIcon />
          </IconButton >
        </Grid>
      </Grid>
